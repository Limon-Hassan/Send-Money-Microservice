...
const providerLogoMap: Record<string, string> = {
  'bKash': '/bank/BKash.svg',
  'bKash Limited': '/bank/BKash.svg',
  'Nagad': '/bank/Nagad.svg',
  'Nagad Limited': '/bank/Nagad.svg',
  'Rocket': '/bank/Rocket.svg',
  'Rocket (DBBL)': '/bank/Rocket.svg',
  'Easypaisa': '/bank/Rocket.svg',
  'Telenor Microfinance Bank': '/bank/Rocket.svg',
};

function ProviderLogo({ name, size = 'w-9 h-9' }: { name: string; size?: string }) {
  const src = providerLogoMap[name];
  if (src) {
    return (
      <img
        src={src}
        alt={name}
        className={`${size} rounded-lg object-contain shrink-0 bg-white`}
      />
    );
  }
  return null;
              ...
              >
                <td className="px-4 py-2.5 whitespace-nowrap">
                  <div className="flex items-center gap-2.5">
                    {providerLogoMap[m.name] || providerLogoMap[m.provider] ? (
                      <ProviderLogo name={providerLogoMap[m.name] ? m.name : m.provider} />
                    ) : (
                      (() => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
      ...
      <div className="flex items-center gap-3 mb-4">
        {providerLogoMap[method.name] || providerLogoMap[method.provider] ? (
          <ProviderLogo name={providerLogoMap[method.name] ? method.name : method.provider} size="w-10 h-10" />
        ) : (
...
export default function MobileWalletPage() {
  const type: PaymentMethodType = 'Mobile Wallet';
