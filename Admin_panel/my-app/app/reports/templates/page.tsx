...
import {
    reportTemplatesPageStats,
    reportTemplateRows,
    reportTemplateRowsTotalCount,
    reportTemplateCategoryFilterOptions,
    reportTemplateOwnerFilterOptions,
    type ReportTemplatesStat,
...
const STAT_ICONS: Record<string, React.ReactNode> = {
    'total-templates': <LayoutTemplate size={16} />,
    'system-templates': <Lock size={16} />,
    'custom-templates': <Wand2 size={16} />,
    'most-used-template': <TrendingUp size={16} />,
};

const STAT_COLORS: Record<string, { bg: string; color: string }> = {
    'total-templates': { bg: 'bg-blue-100 dark:bg-blue-900/40', color: 'text-blue-600 dark:text-blue-400' },
    'system-templates': { bg: 'bg-gray-100 dark:bg-gray-700', color: 'text-gray-500 dark:text-gray-300' },
    'custom-templates': { bg: 'bg-purple-100 dark:bg-purple-900/40', color: 'text-purple-600 dark:text-purple-400' },
    'most-used-template': { bg: 'bg-amber-100 dark:bg-amber-900/40', color: 'text-amber-600 dark:text-amber-400' },
};

function StatCard({ stat }: { stat: ReportTemplatesStat }) {
    return (
...
function CategoryBadge({ category }: { category: string }) {
    const classes: Record<string, string> = {
        Transactions: 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-400',
        Finance: 'bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-400',
        Agents: 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400',
        Compliance: 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400',
        Customers: 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400',
    };
    return (
        <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ${classes[category] ?? 'bg-gray-100 text-gray-600'}`}>
            {category}
        </span>
    );
}
...
function OwnerBadge({ owner }: { owner: string }) {
    ...
    return (
        <span
            className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-medium ${isSystem
                    ? 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300'
                    : 'bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-400'
                }`}
        >
            {isSystem && <Lock size={10} />} {owner}
        </span>
    );
}
...
export default function ReportTemplatesPage() {
    ...
    const filteredRows: ReportTemplateRow[] = useMemo(() => {
        return reportTemplateRows.filter((r) => {
            const matchesCategory = categoryFilter === 'All Categories' || r.category === categoryFilter;
            const matchesOwner = ownerFilter === 'All Owners' || r.owner === ownerFilter;
            const matchesSearch = search.trim() === '' || r.templateName.toLowerCase().includes(search.toLowerCase());
            return matchesCategory && matchesOwner && matchesSearch;
        });
    }, [categoryFilter, ownerFilter, search]);
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-4 dark:bg-gray-900 sm:p-6">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <h1 className="text-xl font-semibold text-gray-900 dark:text-white sm:text-2xl">Report Templates</h1>
                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        Reusable templates to quickly generate consistent, pre-configured reports.
                    </p>
                </div>
            ...
            <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                {reportTemplatesPageStats.map((stat) => (
            ...
            <div className="mt-5 rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800 sm:p-5">
                <h2 className="text-base font-semibold text-gray-900 dark:text-white">All Templates</h2>
                ...
                <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-2">
                    ...
                    >
                        {reportTemplateCategoryFilterOptions.map((c) => (
                    ...
                    <select
                        ...
                        className="rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-700 focus:border-blue-500 focus:outline-none dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200"
                    >
                        {reportTemplateOwnerFilterOptions.map((o) => (
                    ...
                    <table className="min-w-210 w-full text-left text-sm">
                        <thead>
                            <tr className="border-b border-gray-200 text-xs uppercase text-gray-500 dark:border-gray-700 dark:text-gray-400">
                                ...
                                <th className="py-2 pr-4 font-medium">Usage Count</th>
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                            {paginatedRows.map((r) => (
                                <tr key={r.id} className="text-gray-700 dark:text-gray-200">
                                    <td className="py-2.5 pr-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">{r.templateName}</td>
                                    ...
                                    <td className="py-2.5 pr-4">{r.fieldsCount}</td>
                                    <td className="py-2.5 pr-4">{r.usageCount.toLocaleString()}</td>
                                    <td className="py-2.5 pr-4 whitespace-nowrap">{r.lastUsed}</td>
                                    <td className="relative py-2.5 pr-2 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            <button className="text-gray-400 hover:text-blue-600 dark:hover:text-blue-400" title="Use Template">
                                                <Play size={14} />
                                            </button>
                                            <button className="text-gray-400 hover:text-blue-600 dark:hover:text-blue-400" title="Duplicate">
                                                <Copy size={14} />
                                            </button>
                                            <button
                ...
                <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                        ...
                        {Math.min(currentPage * pageSize, filteredRows.length)} of {reportTemplateRowsTotalCount} templates
                    </p>
