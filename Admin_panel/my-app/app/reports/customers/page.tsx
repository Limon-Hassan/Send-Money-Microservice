...
import {
    customerReportsPageStats,
    customerReportRows,
    customerReportRowsTotalCount,
    customerReportCountryFilterOptions,
    customerReportKycFilterOptions,
    customerReportSegmentFilterOptions,
    type CustomerReportsStat,
...
const STAT_ICONS: Record<string, React.ReactNode> = {
    'total-customers': <Users size={16} />,
    'new-customers': <UserPlus size={16} />,
    'active-customers': <UserCheck size={16} />,
    'kyc-verified': <ShieldCheck size={16} />,
};

const STAT_COLORS: Record<string, { bg: string; color: string }> = {
    'total-customers': { bg: 'bg-blue-100 dark:bg-blue-900/40', color: 'text-blue-600 dark:text-blue-400' },
    'new-customers': { bg: 'bg-purple-100 dark:bg-purple-900/40', color: 'text-purple-600 dark:text-purple-400' },
    'active-customers': { bg: 'bg-green-100 dark:bg-green-900/40', color: 'text-green-600 dark:text-green-400' },
    'kyc-verified': { bg: 'bg-amber-100 dark:bg-amber-900/40', color: 'text-amber-600 dark:text-amber-400' },
};

function StatCard({ stat }: { stat: CustomerReportsStat }) {
...
function SegmentBadge({ segment }: { segment: string }) {
    ...
    return <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ${classes}`}>{segment}</span>;
}
...
export default function CustomerReportsPage() {
    ...
    const filteredRows: CustomerReportRow[] = useMemo(() => {
        return customerReportRows.filter((r) => {
            const matchesCountry = countryFilter === 'All Countries' || r.country === countryFilter;
            const matchesKyc = kycFilter === 'All KYC Status' || r.kycStatus === kycFilter;
            ...
            return matchesCountry && matchesKyc && matchesSegment && matchesSearch;
        });
    }, [countryFilter, kycFilter, segmentFilter, search]);
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-4 dark:bg-gray-900 sm:p-6">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <h1 className="text-xl font-semibold text-gray-900 dark:text-white sm:text-2xl">Customer Reports</h1>
                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        Analyze customer base, KYC verification status, and engagement segments.
                    </p>
                </div>
            ...
            <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                {customerReportsPageStats.map((stat) => (
            ...
            <div className="mt-5 rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800 sm:p-5">
                <h2 className="text-base font-semibold text-gray-900 dark:text-white">All Customers</h2>
                ...
                <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-2">
                    ...
                    >
                        {customerReportCountryFilterOptions.map((c) => (
                    ...
                    <select
                        ...
                        className="rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-700 focus:border-blue-500 focus:outline-none dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200"
                    >
                        {customerReportKycFilterOptions.map((k) => (
                    ...
                    <select
                        ...
                        className="rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-700 focus:border-blue-500 focus:outline-none dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200"
                    >
                        {customerReportSegmentFilterOptions.map((s) => (
                    ...
                    <table className="min-w-225 w-full text-left text-sm">
                        <thead>
                            <tr className="border-b border-gray-200 text-xs uppercase text-gray-500 dark:border-gray-700 dark:text-gray-400">
                                ...
                                <th className="py-2 pr-4 font-medium">KYC Status</th>
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                            {paginatedRows.map((r) => (
                                <tr key={r.id} className="text-gray-700 dark:text-gray-200">
                                    <td className="py-2.5 pr-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">{r.customerId}</td>
                                    ...
                                    <td className="py-2.5 pr-4">{r.totalTransactions}</td>
                                    ...
                                    <td className="py-2.5 pr-4 whitespace-nowrap">{r.registrationDate}</td>
                            ...
                            {paginatedRows.length === 0 && (
                                <tr>
                                    <td colSpan={9} className="py-6 text-center text-sm text-gray-400 dark:text-gray-500">
                ...
                <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                        ...
                        {Math.min(currentPage * pageSize, filteredRows.length)} of {customerReportRowsTotalCount.toLocaleString()} customers
                    </p>
