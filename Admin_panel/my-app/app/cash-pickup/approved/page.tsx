...
import {
    ChevronRight, Search, ChevronDown, Filter, Eye, CheckCircle2, XCircle,
    Calendar, ClipboardCheck, CalendarClock, Hourglass,
} from 'lucide-react';
import {
    approvedPickupRequests,
    approvedStatsCashPickup,
    ApprovedPickupRequest,
} from '@/lib/data';
...
// ── Stat cards ────────────────────────────────────────────────
function StatCards() {
    const s = approvedStatsCashPickup;
    const cards = [
        { label: 'Total Approved', value: s.totalApproved, icon: ClipboardCheck, color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-950' },
        { label: 'Scheduled Today', value: s.scheduledToday, icon: CalendarClock, color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-50 dark:bg-blue-950' },
        { label: 'Awaiting Pickup', value: s.awaitingPickup, icon: Hourglass, color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-950' },
        { label: 'Avg Approval Time', value: `${s.avgApprovalMins}m`, icon: ClipboardCheck, color: 'text-purple-600 dark:text-purple-400', bg: 'bg-purple-50 dark:bg-purple-950' },
    ];
    return (
...
function ConfirmModal({
    ...
    request: ApprovedPickupRequest; action: 'complete' | 'cancel';
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" onClick={onClose}>
            <div className="bg-white dark:bg-gray-800 rounded-xl w-full max-w-sm p-5" onClick={e => e.stopPropagation()}>
                <span className={`w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-3 ${isComplete ? 'bg-violet-50 text-violet-600 dark:bg-violet-950 dark:text-violet-400' : 'bg-red-50 text-red-600 dark:bg-red-950 dark:text-red-400'
                    }`}>
                    {isComplete ? <CheckCircle2 size={22} /> : <XCircle size={22} />}
                </span>
                <h3 className="text-[14px] font-semibold text-gray-900 dark:text-white text-center mb-1">
                    {isComplete ? 'Mark as Completed?' : 'Cancel this request?'}
                </h3>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        onClick={onConfirm}
                        className={`flex-1 px-3 py-2 rounded-lg text-white text-[12px] font-medium cursor-pointer ${isComplete ? 'bg-violet-600 hover:bg-violet-700' : 'bg-red-600 hover:bg-red-700'
                            }`}
                    >
                        {isComplete ? 'Mark Completed' : 'Cancel Request'}
                    </button>
                </div>
            </div>
        </div>
...
export default function ApprovedRequestsPage() {
    const [requests, setRequests] = useState<ApprovedPickupRequest[]>(approvedPickupRequests);
    const [search, setSearch] = useState('');
    const [confirmAction, setConfirmAction] = useState<{ request: ApprovedPickupRequest; action: 'complete' | 'cancel' } | null>(null);
    ...
    const handleConfirm = () => {
        ...
        showToast(`${request.id} ${action === 'complete' ? 'marked as completed' : 'cancelled'}`);
    ...
    return (
        <div className="px-4 py-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
            {/* header */}
            <div className="mb-5">
                <h1 className="text-xl font-bold text-gray-900 dark:text-white">Approved Requests</h1>
                <div className="flex items-center gap-2 text-[12px] text-gray-400 dark:text-gray-500 mt-1">
                    ...
                    <span className="text-gray-700 dark:text-gray-300 font-medium">Approved Requests</span>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl">
                    ...
                    <table className="w-full text-left min-w-[960px]">
                        <thead>
                            <tr className="text-[10px] uppercase tracking-wider text-gray-400 dark:text-gray-500 border-b border-gray-100 dark:border-gray-700/60">
                                ...
                                <th className="px-2 py-2.5 font-medium">Approved By</th>
                                <th className="px-2 py-2.5 font-medium">Approved At</th>
                                <th className="px-2 py-2.5 font-medium">Scheduled Pickup</th>
                                ...
                                <tr key={r.id} className="border-b border-gray-50 dark:border-gray-700/40 last:border-0 hover:bg-gray-50/60 dark:hover:bg-gray-700/30">
                                    ...
                                    <td className="px-2 py-2.5 whitespace-nowrap">
                                        <span className="inline-flex items-center gap-1.5 text-[12px] text-gray-600 dark:text-gray-300">
                                            <span className="w-5 h-5 rounded-full bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 text-[9px] font-semibold flex items-center justify-center">
                                                {r.approvedBy.split(' ').map(n => n[0]).join('')}
                                            </span>
                                            {r.approvedBy}
                                        </span>
                                    </td>
                                    <td className="px-2 py-2.5 text-[12px] text-gray-500 dark:text-gray-400 whitespace-nowrap">{fmtDateTime(r.approvedAt)}</td>
                                    <td className="px-2 py-2.5 text-[12px] text-gray-600 dark:text-gray-300 whitespace-nowrap">{r.scheduledPickupDate}</td>
                                    <td className="px-2 py-2.5 text-right whitespace-nowrap">
                                        <div className="flex items-center justify-end gap-1.5">
                                            ...
                                            <button
                                                onClick={() => setConfirmAction({ request: r, action: 'complete' })}
                                                className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-violet-50 dark:bg-violet-950 text-violet-600 dark:text-violet-400 text-[11px] font-medium hover:bg-violet-100 dark:hover:bg-violet-900 cursor-pointer"
                                            >
                                                <CheckCircle2 size={11} /> Complete
                                            </button>
                                            <button
                                                onClick={() => setConfirmAction({ request: r, action: 'cancel' })}
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-4 py-3 text-[12px] text-gray-500 dark:text-gray-400">
                    <span>Showing 1 to {filteredRows.length} of {approvedStatsCashPickup.totalApproved} approved requests</span>
                    <div className="flex items-center gap-1">
                        ...
                        <button className="px-2.5 py-1 rounded border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer">57</button>
                        <button className="px-2 py-1 rounded border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer">›</button>
                    </div>
